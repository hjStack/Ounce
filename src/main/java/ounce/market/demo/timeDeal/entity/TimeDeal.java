...
public class TimeDeal extends BaseEntity {
    ...
    @Column(nullable = false)
    private int maxPurchaseLimit; // 타임딜용 한정 수량 (예: 선착순 50 개!)

    @Builder
    public TimeDeal(Product product, int discountRate, LocalDateTime startTime,
                    LocalDateTime endTime, int maxPurchaseLimit) {
        this.product = product;
        this.discountRate = discountRate;
        this.startTime = startTime;
        this.endTime = endTime;
        this.maxPurchaseLimit = maxPurchaseLimit;
        this.status = DealStatus.READY;  // 안 주면 READY
    }
