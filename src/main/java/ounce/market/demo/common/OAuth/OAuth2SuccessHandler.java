...
//     private final JwtProvider jwtProvider; // JWT 생성기 의존성 주입 (주석 해제 필요)
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        String token = "temp-jwt-token-12345"; // 테스트용 임시 토큰 (나중에 위 코드로 변경)

        // 3. 프론트엔드(클라이언트)로 토큰을 전달하기 위한 URL 세팅
        // 프론트엔드의 메인 페이지(또는 리다이렉트 전용 페이지) 뒤에 ?token=토큰값 형태로 붙여서 보냅니다.
        String targetUrl = UriComponentsBuilder.fromUriString(frontendUrl) // Ounce 메인 주소
 ...
 * 쿠키란
 ...
 인터넷 통신(HTTP)은 본래 '무상태(Stateless)'입니다. 즉, 브라우저가 서버에 요청을 보내고 응답을 받으면 통신이 끊어지고, 서버는 방금 자기가 누구랑 대화했는지 1초 만에 까먹어버립니다. (완벽한 기억 상실증이죠.)
